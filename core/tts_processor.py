...
class TTSProcessor:
    ...
    def generate_tts_from_reference(self, reference_results_path: str, output_dir: Optional[str] = None) -> Dict[
        str, Any]:
        """
        根据参考音频结果生成TTS语音
        ...
        try:
            ...
            if not os.path.exists(reference_results_path):
                return {'success': False, 'error': f'参考音频结果文件不存在: {reference_results_path}'}
            ...
            self.logger.info(f"TTS生成完成: {successful_segments}/{len(reference_segments)} 个片段成功")
    ...
    def _generate_single_tts(self, reference_segment: Dict[str, Any], output_dir: Path) -> Optional[Dict[str, Any]]:
        """生成单个TTS音频片段"""
        try:
            ...
            if tts_result and tts_path.exists():
                ...
                if isinstance(tts_result, dict):
                    result.update({
                        ...
                        'generated_silence': tts_result.get('generated_silence', False),
    ...
    def _call_tts_generation(self, text: str, output_path: str, reference_file: str = None) -> Optional[Dict[str, Any]]:
        """调用TTS模型生成音频"""
        try:
    ...
    def _call_gradio_tts(self, text: str, output_path: str, reference_file: str = None) -> Optional[Dict[str, Any]]:
        """调用Gradio Index-TTS API生成音频"""
        try:
            self.logger.debug(f"调用Gradio Index-TTS: '{text[:30]}...' -> {output_path}")
    ...
    def _generate_silence_audio(self, output_path: str, duration: float) -> Dict[str, Any]:
        """生成静音音频
    ...
    def _check_tts_cache(self, reference_path: str, output_dir: str) -> Optional[Dict[str, Any]]:
        """检查TTS缓存"""
        try:
            # 构建缓存文件路径
    ...
    def _is_tts_cache_valid(self, cached_data: Dict[str, Any], reference_path: str) -> bool:
        """验证TTS缓存是否有效"""
        try:
            # 检查基本字段
            ...
            if cache_time_str:
                try:
                    cache_time = datetime.datetime.fromisoformat(cache_time_str).timestamp()
