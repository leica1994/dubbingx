...
class StepProgressDetail:
    ...
    def update_progress(self, current: int, total: int, item_data: Optional[Dict[str, Any]] = None):
        """更新进度"""
    ...
    def is_completed(self) -> bool:
        ...
        return (self.status == StepStatus.COMPLETED and 
                self.current_item >= self.total_items and 
                len(self.failed_items) == 0)
...
class Task:
    ...
    step_details: Dict[int, StepProgressDetail] = field(default_factory=dict)  # 新增：步骤详细信息
    ...
    def init_step_detail(self, step_id: int, step_name: str, total_items: int = 0) -> StepProgressDetail:
        """初始化步骤详细信息"""
    ...
    def add_failed_item(self, step_id: int, item_data: Dict[str, Any], error: str) -> None:
        """为指定步骤添加失败项目"""
        if step_id in self.step_details:
    ...
    def save_to_cache(self, cache_file_path: Path) -> bool:
        """保存任务状态到缓存文件"""
        from ..cache import TaskCacheManager, CacheMetadata
    ...
    @classmethod
    def load_from_cache(cls, cache_file_path: Path, video_path: str, 
                       subtitle_path: Optional[str] = None) -> Optional["Task"]:
        """从缓存文件加载任务状态"""
        from ..cache import TaskCacheManager
        
        cache_manager = TaskCacheManager()
        cache_data = cache_manager.load_task_cache(cache_file_path, video_path, subtitle_path)
