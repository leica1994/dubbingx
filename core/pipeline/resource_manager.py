...
class ResourceManager:
    ...
    def __init__(self, gpu_count: int = 2, cpu_count: int = 4, io_count: int = 8):
        """
        初始化资源管理器
        ...
        self.logger.info(
            f"资源管理器初始化完成: GPU={gpu_count}, CPU={cpu_count}, I/O={io_count}"
        )
            ...
            if acquired:
                ...
                with self._stats_lock:
                    ...
                    if len(self._hold_times[resource_type]) > 1000:
                        self._hold_times[resource_type] = self._hold_times[
                            resource_type
                        ][-1000:]
    ...
    def try_acquire_resource(
        self, resource_type: ResourceType, task_id: Optional[str] = None
    ) -> bool:
        """
        尝试立即获取资源（非阻塞）
        ...
        Returns:
            是否成功获取
        """
        with self.acquire_resource(
            resource_type, timeout=0, task_id=task_id
        ) as acquired:
            return acquired
        ...
        return {
            "resource_stats": {
                rt.value: {
                    ...
                    "usage_rate": (
                        f"{(stats.in_use_count / stats.max_count * 100):.1f}%"
                    ),
            ...
            "current_usage": {
                rt.value: len(usages) for rt, usages in current_usage.items()
            },
