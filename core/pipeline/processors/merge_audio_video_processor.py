...
from ...media_processor import merge_audio_video
...
class MergeAudioVideoProcessor(StepProcessor):
    ...
    def _execute_process(self, task: Task) -> ProcessResult:
        ...
        try:
            ...
            if not speed_adjusted_video_path or not Path(speed_adjusted_video_path).exists():
                return ProcessResult(
                    success=False,
            ...
            self.logger.info(f"开始音视频合并: {speed_adjusted_video_path} + {aligned_audio_path}")
            ...
            self.logger.info(f"音视频合并完成: {final_video_path} ({file_size_mb:.1f}MB)")
